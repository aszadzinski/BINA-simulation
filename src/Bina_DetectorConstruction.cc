...
//--------- Definitions of Solids, Logical Volumes, Physical Volumes ---------
								...
								z_shift = box_z - box_z_vac - 10*eps_separ; // shift of positions for objects placed in vacuum
																...
																do
																{
																		...
																		if (i%3 == 1) // anode
																		{
																			...
																			while ((delta = sqrt(0.25*mwpcholeOut*mwpcholeOut - 0.25*temp2*temp2)) >= 0.)
																			{
																							...
																							mwpc1_wire_x_phs[j] = new G4PVPlacement(plane_x,position,"Mwpc1_wire_x_c_u_physical",
																																											mwpc1_wire_c_log[k],mwpc1_wire_plane_mother_phs[i],false,j); ++j;
																							mwpc1_wire_x_phs[j] = new G4PVPlacement(plane_x,-position,"Mwpc1_wire_x_c_u_physical",
																																											mwpc1_wire_c_log[k],mwpc1_wire_plane_mother_phs[i],false,j); ++j;
																							position = G4ThreeVector(temp2,-(0.5*mwpcdimgx + delta),0.);
																							mwpc1_wire_x_phs[j] = new G4PVPlacement(plane_x,position,"Mwpc1_wire_x_c_u_physical",
...
//	   while (plac[t].rx>2*M_PI||plac[t].ry>2*M_PI||plac[t].z>2*M_PI);
																									...
																									if(plac[t].typ==0||plac[t].typ==1||plac[t].typ==2||plac[t].typ==3|| plac[t].typ==4||plac[t].typ==5||plac[t].typ==6)
																									{
																											...
																											Edet_B_phs[gE]=new G4PVPlacement(rotBin, G4ThreeVector(xb, yb, zb),"EDet_BINA",
																																											Edet_B_log[plac[t].typ],  Mars_vac_phs,false,NumBina[gE]); gE++;
																									...
																									if(plac[t].typ==10||plac[t].typ==11||plac[t].typ==12||plac[t].typ==13|| plac[t].typ==14||plac[t].typ==15||plac[t].typ==16)
																									{
																											...
																											deltaE_B_phs[gdE]=new G4PVPlacement(rotBin, G4ThreeVector(xb, yb, zb),"DeltaE_BINA",
																																													deltaE_B_log[nrde], Mars_vac_phs,false,NumBina[gdE]);  gdE++;
																									}
																									rotBin++;
																								}
																}
